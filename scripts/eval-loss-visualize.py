...
FILTER_CONST = 50
SUBSAMPLE=100
OUTPUT_DIR = f'/nlp/scr/kathli/eval/rep_struct/figs_genial_sea'
USE_FILTER = True
...
def create_graph_and_visualization(curr_ce_f, curr_ce_s, my_cross_entr, tokens, i, use_filter=False, single_specific=None):
    START_IDX = 0
    ...
    my_cross_entr = my_cross_entr[START_IDX:PRINT_UP_TO_IDX]
    should_print = False
    ...
    if use_filter:
        ...
        plot_l = lfilter(b, a, my_cross_entr)
    
    plt1.plot(indices, plot_f, label=SHORTF_MODEL_LABEL, color='blue', alpha=0.7)
    plt1.plot(indices, plot_s, label=SHORTS_MODEL_LABEL, color='green', alpha=0.7)
    plt1.plot(indices, plot_l, label=LONG_MODEL_LABEL, color='red', alpha=0.7)
    ...
    if single_specific:
        ...
        plt1.scatter(marker_indices, marker_ce_f, color='blue', alpha=0.7)
        plt1.scatter(marker_indices, marker_ce_s, color='green', alpha=0.7)
        plt1.scatter(marker_indices, marker_my_ce, color='red', alpha=0.7)
    plt1.legend()
    ...
    fig.savefig(f'{OUTPUT_DIR}/{DATAFILE}_{SHORTF_MODEL_NAME}_{SHORTS_MODEL_NAME}_{LONG_MODEL_NAME}_{i}.png', dpi=300)
    fig.show()


def create_graph_and_visualization_idx(curr_ce_f, curr_ce_s, my_cross_entr, tokens, i, idx, use_filter=False):
    START_IDX = 0
    ...
    if use_filter:
        ...
        b = [1.0 / n] * n
        a = 1
        plt1.plot(indices, lfilter(b, a, curr_ce_f[idx::3]), label=SHORTF_MODEL_LABEL, color='blue', alpha=0.7)
        plt1.plot(indices, lfilter(b, a, curr_ce_s[idx::3]), label=SHORTS_MODEL_LABEL, color='green', alpha=0.7)
        plt1.plot(indices, lfilter(b, a, my_cross_entr[idx::3]), label=LONG_MODEL_LABEL, color='red', alpha=0.7)
    else:
        plt1.plot(indices, curr_ce_f[idx::3], label=SHORTF_MODEL_LABEL, color='blue', alpha=0.7)
        plt1.plot(indices, curr_ce_s[idx::3], label=SHORTS_MODEL_LABEL, color='green', alpha=0.7)
        plt1.plot(indices, my_cross_entr[idx::3], label=LONG_MODEL_LABEL, color='red', alpha=0.7)
    plt1.legend()
    plt1.set_xlabel('Time')
    plt1.set_ylabel('cross_entr')
    ...
    # Save all four plots to image files
    fig.savefig(f'{OUTPUT_DIR}/{DATAFILE}_{SHORTF_MODEL_NAME}_{SHORTS_MODEL_NAME}_{LONG_MODEL_NAME}_{i}_{idx}_3.png', dpi=300)
    fig.show()
